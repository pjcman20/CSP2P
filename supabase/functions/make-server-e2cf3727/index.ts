...
app.post("/make-server-e2cf3727/offers/create", authMiddleware, async (c) => {
  try {
    ...
    const formattedOffer = {
      id: offer.id,
      userId: steamId,
      userName: authUser.user_metadata?.persona_name || existingProfile?.persona_name || 'Steam User',
      userAvatar: authUser.user_metadata?.avatar_url || existingProfile?.avatar_url || '',
      userTradeUrl: tradeUrl,
      userProfileUrl: authUser.user_metadata?.profile_url || existingProfile?.profile_url || '',
...
app.get("/make-server-e2cf3727/requests/received", authMiddleware, async (c) => {
  try {
    ...
    const { data: requests, error } = await supabase
      ...
      `)
      .eq('offer_owner_steam_id', steamId)
      .order('created_at', { ascending: false });
    ...
    // Get requester profile data for all requesters
    const requesterSteamIds = [...new Set(requests.map((r: any) => r.requester_steam_id))];
    const { data: requesterProfiles } = await supabase
      .from('users')
      .select('steam_id, persona_name, avatar_url')
      .in('steam_id', requesterSteamIds);

    const profileMap = new Map();
    if (requesterProfiles) {
      requesterProfiles.forEach((profile: any) => {
        profileMap.set(profile.steam_id, profile);
      });
    }
    ...
    const formattedRequests = requests.map((request: any) => {
      const requesterProfile = profileMap.get(request.requester_steam_id);
      return {
        id: request.id,
        offerId: request.offer_id,
        requesterId: request.requester_steam_id,
        requesterName: requesterProfile?.persona_name || 'Steam User',
